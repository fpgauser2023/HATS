package UFS.PiecewiseTaylor.Multicycle
...
class UFSPTConstraints(val int_width: Int, val rat_width: Int, val total_inner_regions: Int, val order_of_approx: Int) {

    println("\nGenerating Constraints")

    // Total bits available in entire format.
    val bit_width: Int = int_width + rat_width
    val region_width: Int = scala.math.ceil(log10(total_inner_regions+2+1)/log10(2)).toInt
    ...
    val flt_boundaries: Seq[Double] = generateBoundaries(total_inner_regions)
    ...
    def generateHorizontalShifts(boundaries: Seq[Double]): Seq[Double] = {
        ...
        Seq(-7.0, -4.7) ++ innerAs ++ Seq(4.7, 7.0)
    }
    ...
    def calculateCoefficient(midpoint_of_region: Double, order_of_approximation: Int): Double = {
        ...
        if (midpoint_of_region <= -6) {
            0.0
        } else if (midpoint_of_region <= -3.4) {
            order_of_approximation match {
                case 0 => flt_0_071
                case 1 => flt_0_011
                case _ => 0.0
            }
        } else if (midpoint_of_region <= 0) {
            order_of_approximation match {
                case 0 => (0.25*midpoint_of_region+1)/(2-midpoint_of_region)
                case _ => 1.5/pow(2-midpoint_of_region, 1+order_of_approximation)
            }
        } else if (midpoint_of_region <= 3.4) {
            order_of_approximation match {
                case 0 => (1.25*midpoint_of_region+1)/(2+midpoint_of_region)
                case _ => 1.5/pow(-midpoint_of_region-2,1+order_of_approximation)
            }
...
class CheckBoundary(val constraints: UFSPTConstraints) extends Module {
...
class BoundarySelect(val constraints: UFSPTConstraints) extends Module {
...
class ConstantSelect(val constraints: UFSPTConstraints) extends Module {
...
class TaylorExpansion(val constraints: UFSPTConstraints) extends Module {
    ...
    val exponentiatial_accumulator: SInt = RegInit(0.S(constraints.bit_width.W))
    ...
    // internal wiring
    product_result := (scalar * exponentiatial_accumulator) >> constraints.rat_width
    exponential_accumulator_output := (exponentiatial_accumulator * processed_input) >> constraints.rat_width
    io.output := product_accumulator

    when (io.initialize) {
        processed_input := io.xa
        exponentiatial_accumulator := constraints.mapToFixedPoint(1).S
        scalar := io.mrn
        product_accumulator := 0.S
    } .elsewhen(io.compute_enable) {
        processed_input := processed_input
        exponentiatial_accumulator := exponential_accumulator_output
        scalar := io.mrn
        product_accumulator := product_result + product_accumulator
    } .otherwise {
        processed_input := processed_input
        exponentiatial_accumulator := exponentiatial_accumulator
        scalar := 0.S
...
class UFSPT(val constraints: UFSPTConstraints) extends Module {
...
//    taylorExpansion.io.xa := Mux(constantSelect.io.sel, io.input, constantSelect.io.a_select + io.input)
    taylorExpansion.io.xa := constantSelect.io.a_select + io.input
...
object GenerateVerilog extends App {
    ...
    val output_file_name: String = "UFSPT-%d.%d-%dRegions-Order%d".format(
        integerWidth, rationalWidth, numberOfInnerRegions, orderOfTaylorApproximations
    )

    //// Module Generation
    val constraints: UFSPTConstraints = new UFSPTConstraints(integerWidth, rationalWidth,
        numberOfInnerRegions, orderOfTaylorApproximations)
    (new ChiselStage).emitVerilog(
        new UFSPT(constraints),
