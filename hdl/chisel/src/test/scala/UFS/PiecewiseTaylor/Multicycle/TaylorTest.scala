...
class TestTaylor extends AnyFlatSpec with ChiselScalatestTester with Matchers {
    behavior of "UFSTaylor"

    // Configure UFS-PT
    val int_width = 5
    val rat_width = 10
    val number_of_inner_regions = 8
    val order_of_approx = 3
    ...
    it should "do something" in {
        // test case body here
        val constraints_inst = new UFSPTConstraints(int_width, rat_width, number_of_inner_regions, order_of_approx)
        test(new UFSPT(constraints_inst)) { c =>
