...
import chisel3._
import chiseltest._
import org.scalatest.{FixtureSuite, FlatSpec}
...
class TestUfsptMulticycle extends FlatSpec with ChiselScalatestTester with Matchers {
    ...
    val constraints: UFSPTConstraints = new UFSPTConstraints(fpiFormat(0), fpiFormat(1), numberOfRegions, orderOfApproximation)
    logFileHandler.write("$\\widetilde{\\sigma}_{UFS-PT}(x)$ (Multicycle) -- I=%d, F=%d, R=%d, d=%d\n".format(fpiFormat(0), fpiFormat(1), numberOfRegions, orderOfApproximation))
